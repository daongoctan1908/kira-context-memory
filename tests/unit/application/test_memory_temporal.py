...
def test_each_message_uses_its_own_utc_day_across_midnight_and_month_end():
    messages = (
        user("Hôm nay ưu tiên Hà Nội.", timestamp="2026-09-30T23:59:00Z"),
        assistant("Đã rõ.", timestamp="2026-10-01T00:01:00Z"),
    )
    table = source_table(build_memory_extraction_prompt(messages))
    assert table == {"source_time": ["2026-09-30T23:59:00+00:00", "2026-10-01T00:01:00+00:00"]}
...
def test_missing_timestamps_remain_null_and_do_not_shift_subsequent_indices():
    ...
    assert source_table(prompt) == {
        "source_time": ["2026-10-01T00:00:00+00:00", None, "2026-09-30T09:00:00+00:00"]
    }
...
def test_message_text_cannot_supply_metadata_or_create_extra_table_rows():
    ...
    assert table == {"source_time": ["2026-09-14T09:00:00+00:00"]}
...
def test_earliest_source_date_needs_no_calendar_arithmetic():
    prompt = build_memory_extraction_prompt((user("test", timestamp="0001-01-01T00:00:00Z"),))
